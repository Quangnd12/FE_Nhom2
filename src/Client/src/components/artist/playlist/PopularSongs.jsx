...
import { getArtistsSong, UPLOADS_BASE_URL } from "../../../config/apiConfig";
...
const PopularSongs = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const songData = await getArtistsSong();
        // Lọc các bài hát của nghệ sĩ có ID tương ứng và loại bỏ trùng lặp
        const filteredSongs = songData.filter(
          (item) => item.artists_id === parseInt(id)
        );
